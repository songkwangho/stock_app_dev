...
- **빈 상태 (14차)**: 서버 `syncAllStocks`가 완료되기 전에는 `market_opinion='긍정적'` 종목이 없어 리스트가 비어 있을 수 있다. 이때 "지금 데이터를 분석 중이에요. 하루 1회(오전 8시) 97종목을 분석해 유망 종목을 선정해요. 첫 실행 시에는 전체 분석이 끝날 때까지 10~15분 정도 걸릴 수 있어요." 안내 카드로 표시.
...
- **알림 출처 뱃지 (14차)**: `alert.source`가 `'holding'`이면 blue `[보유 중]` 뱃지, `'watchlist'`면 purple `[관심 종목]` 뱃지를 종목명 옆에 표시. `source`가 없으면 뱃지 생략 (이전 버전 호환).
