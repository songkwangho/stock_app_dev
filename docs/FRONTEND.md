...
- **빈 상태 (14차 + 16차 5-3 개선)**: 서버 `syncAllStocks`가 완료되기 전에는 `market_opinion='긍정적'` 종목이 없어 리스트가 비어 있을 수 있다. KST 현재 시간 기준 3분기:
...
- 8개 섹터별 종목 그룹 표시
- **카드에 등락률 표시 (16차 5-5)**: 주가 아래에 `▲ +1.2%` 또는 `▼ -0.8%` 표시. `change_rate`가 `"0.00"`/`"+0.00"` placeholder면 숨김. 서버 `stock/service.js`에서 최근 2거래일 종가로 실제 계산 (14차까지는 `"0"`/`"0.00"` 하드코딩이었음).
