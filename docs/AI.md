...
- 기술지표 연속값 스코어링: RSI/MACD/볼린저밴드/거래량 가중 합산
- 수급 스코어링: 외국인/기관 연속 순매수 일수 기반 (investor_history 테이블)
- 보유 종목 5단계 판단: 손절(-7%) → 이중 이평선 이탈 → 관망 → 추가매수 → 보유
...
### 4. 데이터 분석 로직
- 보유종목 5단계 투자의견 판정 (손절/이중이탈/관망/추가매수/보유)
- 비보유종목 10점 만점 통합 스코어링 (밸류에이션+기술지표+수급+추세)
- PER/PBR 섹터 중앙값 비교 밸류에이션
- PEG 비율 계산 (네이버 EPS 스크래핑 기반)
...
### 5. 코드 품질
- TypeScript 인터페이스 정의 (16개, ScoringBreakdown 포함)
