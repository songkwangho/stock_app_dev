...
### 3. 프론트엔드 개발 (src/)
- 전체 UI 컴포넌트 구현 (7개 페이지 + 6개 공용 컴포넌트)
- Tailwind CSS 다크 테마 디자인 시스템
- 반응형 레이아웃: PC 사이드바(`hidden md:flex`) + 모바일 하단 탭바 5개(`fixed bottom-0 md:hidden`)
- Recharts 차트 구현:
  - 라인 차트(기본) / 캔들 차트 토글 (초보자는 라인 차트 기본)
  - 이동평균선 오버레이 + 초보자용 설명 텍스트
  - 투자자 매매동향 바차트
  - 포트폴리오 수익률 에리어차트
  - 자산배분 파이차트
- `ScoringBreakdownPanel`: 10점 스코어를 4영역 게이지 바 + 한국어 해석으로 시각화
- 재무지표 컨텍스트 설명: PER(적자/저평가/고평가), PBR(자산 대비), ROE(수익성 판단), 목표가(상승여력 %)
- 종목 검색 컴포넌트 (디바운스, 드롭다운)
- 종목 스크리너 (프리셋 필터 + 고급 필터)
- 포트폴리오 관리 CRUD (인라인 편집)
  - `holding_opinion` 뱃지 + 이유 텍스트 (매도: 평단가 대비 %, 관망: 20일선, 추가매수: 5일선)
  - 수익률 옆 상황별 격려 메시지 ("잘 하고 계세요!" / "손절 기준에 근접했어요")
- `MarketOpinion` / `HoldingOpinion` 분리 뱃지 UI
- PER 음수 종목 '적자' 뱃지 + 설명 텍스트
- 추천 카드: `source` 뱃지 ('전문가 선정'/'알고리즘') + 신뢰도 tooltip + fairPrice 출처 라벨
- 알림: 아이콘(🔴📉💡🎯💎) + 우선순위(high/medium/low) + 좌측 강조 border
...
### 초보자 친화 UI/UX 개선 (6개 항목)
1. **재무지표 컨텍스트 설명**: PER/PBR/ROE/목표가에 업종 대비 해석 텍스트, 적자 표시, 상승여력 % 병기
2. **`ScoringBreakdownPanel` 신규**: 10점 스코어를 4영역 게이지 바 + 한국어 한줄 해석으로 시각화
3. **알림 강화**: ALERT_TYPE_LABELS에 아이콘(🔴📉💡🎯💎) + priority(high/medium/low) 추가, 높은 우선순위 알림 좌측 border 강조
4. **포트폴리오 의견 이유**: holding_opinion 뱃지 옆에 이유 텍스트(평단가 대비 %, 이평선 상태), 수익률 상황별 격려 메시지
5. **추천 카드 투명성**: source 뱃지를 '전문가 선정'/'알고리즘'으로 변경 + 호버 시 신뢰도 tooltip, fairPrice 출처 라벨(애널리스트/추정) 표기
6. **반응형 레이아웃**: PC 사이드바(`hidden md:flex`) + 모바일 하단 탭바 5개(`fixed bottom-0 md:hidden`), 헤더/검색바/콘텐츠 padding 반응형
7. **차트 토글**: 기본 라인 차트(초보자 친화) + 캔들 차트 전환 버튼, 라인 모드에서 이평선 설명 텍스트
