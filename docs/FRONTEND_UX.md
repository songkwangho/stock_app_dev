...
2. **터치 타겟**: 모든 버튼/탭/아이콘 최소 44x44px (`min-w-[44px] min-h-[44px]` 또는 `p-3` 이상). 16차 5-7: App.tsx 알림 패널의 "지금 확인하기"/"나중에 볼게요" 버튼도 `py-1.5`에서 `py-2 min-h-[44px]`로 교체됨.
...
### 이평선 안내 — StockDetailView 라인 차트 (16차 5-6)
라인 차트 상단 blue 힌트 박스를 "파란선 = 5일 평균" 한 줄에서 **2줄 규격**으로 확장. 초보자가 "평균선"의 의미와 정배열 개념을 모른 채 차트를 보지 않도록 방지.
```
💡 이평선(이동평균선) 보는 법
파란선(5일 평균, 단기 흐름) / 노란선(20일 평균, 중기 흐름).
주가 > 파란선 = 단기 상승 흐름 · 파란선 > 노란선 = 정배열(긍정적 추세).
```
캔들 차트 선택 시에는 기존 `helpTexts.candle` 안내 유지.
...
### 수익률 행동 유도 텍스트 (6구간)
16차 5-1: 손절 기준(-7%) 같은 **특정 숫자를 사용자에게 노출하면 매도 권유로 오해**될 수 있어 문구를 위험 인지 중심으로 순화.
| 구간 | 문구 |
|------|------|
...
| <-7% | "손실이 커지고 있어요. 해당 종목의 분석을 다시 확인해보세요 🔴 [종목 보기 →]" |

### 보유종목 holding_opinion 이유 텍스트
**17차 5-2 레이아웃**: 뱃지(대괄호 라벨)와 이유 텍스트를 줄 분리. 뱃지는 `[주의 필요]`/`[관망]`/`[추가 검토]`/`[보유]`만 표시하고, 이유는 아래 별도 `<p>` 줄로 이동 — 이전 인라인 삽입 방식은 모바일에서 줄바꿈이 지저분했음.

| 상태 | 표시 라벨 | 이유 텍스트 |
|------|----------|-----------|
| 매도(손절, <-7%) | 주의 필요 | "손실이 커지고 있어요. 해당 종목의 분석을 다시 확인해보세요 🔴" (특정 숫자 노출 회피, 17차 5-2) |
| 매도(이탈) | 주의 필요 | "5일·20일 평균 모두 아래로 내려갔어요. 하락 추세예요." |
| 관망 | 관망 | "5일 평균 아래지만 20일 평균이 지지 중이에요. 조금 기다려봐요." |
| 추가매수 | 추가 검토 | "5일 평균 부근에서 지지받고 있어요." |
| 보유 | 보유 | "5일 평균 위, 이평선 정배열이에요. 상승 흐름이 이어지고 있어요." |

### 스코어 임계값 면책 (17차 P4-보완)
`ScoringBreakdownPanel` 상단에 amber 경고 배너 고정 노출: "⚠️ 이 점수 기준은 실증 검증 전이에요. 과거 데이터로 최적화하기 전 임시 기준이니 참고용으로만 봐주세요." — 7/4점 임계값이 Phase 4 백테스팅 완료 전 임시값이므로 앱스토어 심사 리스크 완화 + 초보자 오해 방지.

### 대시보드 수익/손실 해석 힌트 (17차 5-1)
`DashboardPage` 차트 상단에 한 줄 힌트: "💡 평가금액(실선)이 투자원금(파선) 위에 있으면 수익 중, 아래면 손실 중이에요." — 초보자는 Legend만으로 두 라인의 위아래 관계가 수익/손실을 의미한다는 것을 해석하지 못함.

### 주요종목 등락률 기준 안내 (17차 5-3)
`MajorStocksPage` 헤더 하단에 "※ ▲/▼ 등락률은 전일 종가 대비 변동분이에요." 노출 — 기간 기준(오늘? 전체?)을 모르는 초보자를 위한 명시.

### 추천 카드 점수 툴팁 (17차 5-4)
`RecommendedStockCard`의 manual 점수 뱃지(`Zap 92`)에 `?` 기호 + `title` 툴팁으로 "편집팀이 매긴 종목 추천 점수예요. 100점 만점으로, 높을수록 매력적이라고 판단한 종목이에요." 설명. 초보자는 "92"가 무엇 기준 점수인지 모름.

### 스크리너 모바일 지표 힌트 (17차 5-5)
`ScreenerPage` 모바일 카드 PER/PBR/ROE 레이블 옆에 `(낮을수록↓)`/`(1이하↓)`/`(높을수록↑)` 간략 힌트 — PC 테이블에만 있던 해석 정보를 모바일에도 동일 노출.
