...
- **포트**: 3001
- **DB**: SQLite3 (better-sqlite3, 동기식) → PostgreSQL 전환 **진행 중** (14차, Step 1~8 완료)
...
| DB | 빈도 가드 SQL |
|----|---------------|
| **SQLite (현재)** | `SELECT COUNT(*) FROM alerts WHERE device_id = ? AND code = ? AND DATE(created_at, 'localtime') = DATE('now', 'localtime')` |
| **PostgreSQL (전환 후)** | `SELECT COUNT(*) FROM alerts WHERE device_id = $1 AND code = $2 AND (created_at AT TIME ZONE 'Asia/Seoul')::date = (NOW() AT TIME ZONE 'Asia/Seoul')::date` |

> PostgreSQL 전환 시 `'localtime'` modifier가 없으므로 `AT TIME ZONE 'Asia/Seoul'`로 KST 변환 후 `::date` 캐스팅. 이 변경을 놓치면 KST 자정 전후 알림이 잘못 발송될 수 있다.
...
## PostgreSQL 전환 가이드 (Phase 2-1)

### 진행 상황 (14차 — Option B 완료)
**완료된 Step 1~8 (DB 레이어 + 도메인 유틸):**
- ✅ `pg` 패키지 설치
- ✅ `server/db/connection.js` → `pg.Pool` + `query()`/`withTransaction()` 헬퍼
- ✅ `server/db/schema.js` → `TIMESTAMPTZ`, `BIGSERIAL`, `NUMERIC`, `eps_current`/`eps_previous`/`chart_path`/`source`/`category` 초기 DDL 내재화 (PG-1 해소)
- ✅ `server/db/migrate.js` → `information_schema.columns` 기반 검증 로직 (멀티스테이트먼트 제거, PG-2 해소)
- ✅ `server/helpers/queryBuilder.js` 신설 → `buildSetClause`/`buildWhereClause` (PG-4, PG-5 대비)
- ✅ `server/helpers/sma.js` → `async computeSMA(pool, code)`
- ✅ `scoring.js` → `calculateValuationScore`/`calculateTechnicalScore`/`calculateSupplyDemandScore` async 전환 (NUMERIC/BIGINT → `Number()` 캐스팅). `calculateTrendScore`/`calculateHoldingOpinion`은 DB 미접근이라 동기 유지.
- ✅ `alert/service.js` → `generateAlerts` async + 중복 `hasDuplicate`/`dailyLimitReached` 루프 외부 추출 (로직-1 해소) + KST 빈도 가드 SQL `AT TIME ZONE 'Asia/Seoul'::date` 교체

**남은 Step 9~14 (다음 세션 예정 — 서버 기동 불가 상태):**
- ⏳ Step 9: `getStockData()` async 재작성 + `withTransaction` 트랜잭션 교체 (PG-3)
- ⏳ Step 10: 6개 라우터 전체 `db.prepare().get/all/run()` → `await query()` 교체 (28개 엔드포인트, screener/PUT holdings 동적 SQL은 `queryBuilder.js` 활용)
- ⏳ Step 11: `BATCH_SIZE` 5→3 조정 (Neon 풀 크기 고려)
- ⏳ Step 12: Puppeteer (`scrapers/toss.js`) 제거 + `chart_path` 정리
- ⏳ Step 13: SQLite → PostgreSQL 데이터 마이그레이션 + Neon 검증
- ⏳ Step 14: `dataFreshness.ts` 주석 업데이트 (로직-5)

> **현재 상태**: Step 1~8 적용 후 서버는 기동 불가 (라우터가 여전히 `db.prepare` 사용). 다음 세션에서 Step 9~14 완료 전에는 `node server/server.js`를 실행하지 말 것.

### 영향 범위 (사전 조사 결과)
- **better-sqlite3 호출**: 178+ 위치 (`db.prepare`, `db.transaction`, `.run`, `.get`, `.all`) — Step 10에서 일괄 교체
- **동기 스코어링 함수**: Step 7~8에서 3개 async 전환 완료, 2개는 DB 미접근이라 동기 유지 확정
- **`getStockData(code)`**: Step 9 대상. 트랜잭션 2개(`stock_history`, `investor_history` UPSERT) → `withTransaction`로 교체
...
const { rows: [row] } = await query('SELECT * FROM stocks WHERE code = $1', [code]);
...
- 모든 timestamp 컬럼은 `TIMESTAMPTZ` 사용 (NOT `TIMESTAMP WITHOUT TIME ZONE`)
- 클라이언트에는 ISO 8601 (`Z` 접미사 포함)으로 직렬화되어 내려감
- 프론트의 `parseServerDate()`는 SQLite 형식과 ISO 8601 양쪽을 모두 처리하므로 변경 불필요
