...
### 영향 범위 (사전 조사 결과)
- **better-sqlite3 호출**: 178+ 위치 (`db.prepare`, `db.transaction`, `.run`, `.get`, `.all`)
- **동기 스코어링 함수 6개** (모두 `await pool.query()` 기반 async로 재작성 필요):
  - `calculateValuationScore` / `calculateTechnicalScore` / `calculateSupplyDemandScore` / `calculateTrendScore`
  - `calculateHoldingOpinion` (내부에서 DB 호출 없으면 동기 유지 가능)
  - `generateAlerts`
- **`getStockData(code)`**: 이미 async 함수지만 내부 DB 호출이 동기. 전체 함수 await 기반으로 재작성
- **`computeSMA(db, code)`** (`helpers/sma.js`): `async computeSMA(pool, code)`로 시그니처 변경. 호출처(`portfolio/router.js`)에서 await 필요
- **PRAGMA 마이그레이션 10개** (`db/migrate.js`): `PRAGMA table_info(...)` → `information_schema.columns` 기반 헬퍼로 교체

### 단계별 작업 (예상 11~13일)
1. **사전 조사 (0.5일)** — `grep -rn "db\.prepare\|db\.transaction\|\.run(\|\.get(\|\.all("` 으로 전수 확인
2. **`db/connection.js` 교체 (0.5일)** — `pg.Pool` 기반. `query(text, params)`, `withTransaction(fn)` 헬퍼 export
3. **`db/schema.js` DDL 재작성 (1일)** — 타입 변환:
   - `INTEGER PRIMARY KEY AUTOINCREMENT` → `BIGSERIAL PRIMARY KEY`
   - `REAL` → `NUMERIC` 또는 `DOUBLE PRECISION`
   - `DATETIME DEFAULT CURRENT_TIMESTAMP` → `TIMESTAMPTZ DEFAULT NOW()`
   - `ON CONFLICT(...)` 구문은 그대로 호환
4. **`db/migrate.js` 재작성 (1일)** — `columnExists(pool, table, column)` 헬퍼로 PRAGMA 대체
5. **스코어링 함수 6개 async 전환 (2~3일)**
6. **`getStockData()` async 재작성 (2일)**
7. **`generateAlerts()` async 전환 + KST 빈도 가드 SQL 교체 (1일)**
8. **6개 라우터 파일 전체 교체 (2일)** — 28개 엔드포인트의 `db.prepare().get/all/run()` → `await pool.query()`
9. **데이터 마이그레이션 (0.5일)** — `sqlite3 .dump` → 수동 조정 → `psql $DATABASE_URL <`
10. **검증 + Neon/Render 배포 (1~2일)**
