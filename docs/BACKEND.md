...
**stocks**: code(PK), name, category, price, change, change_rate, per, pbr, roe, target_price, eps_current, eps_previous, last_updated. `change`/`change_rate`는 `stock/service.js`에서 최근 2거래일 종가 비교로 계산되어 INSERT와 ON CONFLICT UPDATE 양쪽에 반영된다 (UPDATE 절 누락 시 기존 종목의 값이 갱신되지 않음 — 17차 버그-2 보정).
**holding_stocks**: device_id+code(PK), avg_price(**NUMERIC(14,2)**, 분할 매수 평균 소수점 보존), weight, quantity, last_updated — `holding_opinion`은 DB 미저장, API 응답 시 런타임 계산. pg 드라이버가 NUMERIC을 string으로 반환하므로 라우터/service 응답 직전 `Number()` 캐스팅 필수 (`calculateHoldingOpinion` 전달 전에도 포함). 기존 DB에 INTEGER로 남아 있으면 `migrate.js`가 `information_schema.columns`로 감지 후 ALTER로 승격 (17차 설계-1).
