...
async function insertAlert(pool, device_id, code, name, type, message) {
    await pool.query(
        'INSERT INTO alerts (device_id, code, name, type, message) VALUES ($1, $2, $3, $4, $5)',
        [device_id, code, name, type, message]
    );
}
    ...
    for (const { device_id } of holders) {
        ...
        if (sma5) {
            ...
            if (broken && !(await hasDuplicate(pool, device_id, code, 'sma5_break')) && !(await dailyLimitReached(pool, device_id, code))) {
                await insertAlert(pool, device_id, code, name, 'sma5_break',
                ...
                await insertAlert(pool, device_id, code, name, 'sma5_touch',
        ...
        if (sma5 && sma20ForAlert && price < sma5 && price < sma20ForAlert
            ...
            await insertAlert(pool, device_id, code, name, 'sell_signal',
    ...
    if (targetPrice && price > 0) {
        ...
        for (const { device_id } of watchers) {
            if (price >= targetPrice * 0.95
                ...
                && !(await dailyLimitReached(pool, device_id, code))) {
                await insertAlert(pool, device_id, code, name, 'target_near',
            ...
            if (price < targetPrice * 0.7
                ...
                && !(await dailyLimitReached(pool, device_id, code))) {
                await insertAlert(pool, device_id, code, name, 'undervalued',
