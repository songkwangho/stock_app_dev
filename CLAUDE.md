...
- [x] **라우트 분리 완료** (9차): server.js 891줄 → 80줄 + 5개 라우터
- [x] **10차 추가**: system 라우터 분리, cleanupOldData 시드 보존 가드, 알림 메시지 중립적 표현
- [x] **11차 보완**:
  - `computeSMA` → `helpers/sma.js`로 재이동 (도메인 간 단방향 의존성 원칙)
  - 알림 일일 빈도 가드 구현 (`DAILY_ALERT_LIMIT_PER_STOCK = 2`, KST 기준)
  - sma5_break / sma5_touch 경계 중복 수정 (이탈 우선)
  - 지표 가용성 플래그 추가 (`rsi_available` ≥15일 / `macd_available` ≥26일 / `bollinger_available` ≥20일)
1. [ ] **HMAC 서명** (다음 우선순위) — 폴백 모드 설계 + B안 강제 재등록
   - **B안 실행 시 사용자 안내 화면**: "데이터를 이전할 수 없어요. 다시 입력해 주세요" + 간단한 재입력 가이드 (이전 device_id의 holdings 수가 표시될 수 있도록 서버 응답에 카운트만 노출)
   - 온보딩 "직접 추가할게요" 진입 후 서버 미연결 시: 검색 폼 노출 + 추가 시도 시 "서버 연결이 필요해요" 안내 + 재시도
2. [ ] **PostgreSQL 전환 사전 작업**: better-sqlite3 사용 패턴 전수 조사 → syncAllStocks() 비동기 재작성 범위 산정
3. [ ] SQLite → PostgreSQL 전환
4. [ ] KIS/KRX 평가 (시나리오 B 우선: KRX 전일 데이터 + 네이버 보조)

### Phase 3 - 앱 배포 (HMAC 완료 후)
- [ ] Capacitor 설정 + @capacitor/preferences + 오프라인 폴백
- [ ] Push 파이프라인 (PostgreSQL 후): generateAlerts에 일 N건 가드 이미 적용. **알림 본문에 "하루 1회 갱신 기반 알림" 명시** (실시간 아님)
- [ ] **번들 크기 분석** (<250KB gzip 목표). **Recharts 동적 import는 Phase 3 착수와 동시에 진행** — 현재 Recharts vendor 청크가 117KB gzip이라 목표 달성에 필수
- [ ] 실제 디바이스 성능 테스트 (시작 3초 / Phase1 2초 / 탭 300ms)
- [x] **앱스토어 카테고리 결정**: "Utilities" 잠정 결정 (Finance 심사 리스크 회피). 심사 노트에 "정보 제공 도구, 거래 기능 없음" 명시

### Phase 4 - 품질 향상 (즉시 병행 착수)
- [ ] `scripts/backfill-history.js`: 97종목 × 3년 ≈ 72,750건, 종목당 1초 딜레이, 하루 20~30종목 분할, 체크포인트
- [ ] 백테스팅 모듈 + 스코어 임계값 최적화 + 수급 금액 가중치
  - **결과 UI에 "과거 성과가 미래 수익을 보장하지 않습니다" 면책 필수**
- [ ] **섹터별 스코어링 가중치 테이블 도입**: 바이오는 밸류에이션 가중치 ↓ + 수급·추세 가중치 ↑. 금융은 PBR 가중치 ↑. 도메인 한계 (`SKILL_KOREAN_STOCK_APP.md` 5장) 해소
