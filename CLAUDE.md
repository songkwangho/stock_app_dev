...
1. [ ] **PostgreSQL 전환** (이번 단계 메인, 11~13일 — `docs/BACKEND.md` PostgreSQL 전환 가이드 참조)
   - 단계: 사전 조사 → connection.js 교체 → schema.js DDL → migrate.js → 스코어링 6개 async → getStockData async → generateAlerts KST SQL → 라우터 28개 교체 → 데이터 마이그레이션 → 검증
   - **DB 호스팅**: Neon 무료 플랜 ($0). 매일 08:00 syncAllStocks가 sleep 방지
   - **알림 빈도 가드 SQL** PostgreSQL용으로 교체 (`AT TIME ZONE 'Asia/Seoul'::date`)
