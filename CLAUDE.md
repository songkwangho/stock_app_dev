...
2. [ ] **`scripts/backfill-history.js` 작성** (Phase 4 선행). 97종목 × 3년 ≈ 72,750건, 종목당 1초 딜레이, 하루 20~30종목 분할, 체크포인트
3. [ ] **CORS `ALLOWED_ORIGINS` 환경변수화**: 배포 직전 `process.env.FRONTEND_URL` 등으로 분리해 Vercel URL을 코드에 하드코딩하지 않도록 (P3-2).
...
- [ ] `FRONTEND_URL` Render에 Vercel 배포 URL 설정 → CORS `ALLOWED_ORIGINS` 자동 포함
...
8. [ ] **Neon sleep 해제 + syncAllStocks 재시도 (P3-3)**: Neon 무료 플랜 sleep 상태에서 첫 connection 실패 시 `syncAllStocks()` 자체에 backoff 재시도(예: 30초 후 1회 재시도) 추가. 현재는 5초 후 한 번만 시도하고 실패하면 다음 08:00까지 갱신 없음.
...
- [ ] **Toss Payments 구독 연동** (월 3,900원). 결제 콜백 → users.tier 갱신
