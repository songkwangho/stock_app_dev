...
- [x] **13차**: ErrorBanner 공통 컴포넌트, DashboardPage 차트 동적 색상·풀 날짜 라벨, 평단가 폼 초보자 레이블·힌트, MajorStocksPage 삭제 확인 모달, 추천 카드 placeholder 점수 숨김 (algorithm 추천), 서버 health 스플래시 (`/api/health` 게이트)
- [x] **14차 Option B (Step 1~8)**: `generateAlerts` 중복 함수 추출(로직-1), `pg` 설치, `connection.js`→pg.Pool, `schema.js` PG DDL (eps/category/chart_path 내재화, TIMESTAMPTZ, BIGSERIAL — PG-1 해소), `migrate.js` information_schema 기반 (멀티스테이트먼트 제거 — PG-2 해소), `helpers/queryBuilder.js` 신설 (PG-4/5 대비), `sma.js`/스코어링 3개 async, `generateAlerts` async + KST SQL. **서버 기동 불가 상태 — 다음 세션에서 Step 9~14 완료 필요**
1. [ ] **PostgreSQL 전환 Step 9~14** (다음 세션 메인)
   - Step 9: `getStockData()` async + `withTransaction`로 트랜잭션 2곳 교체 (PG-3)
   - Step 10: 6개 라우터 전체 `db.prepare` → `await query()` 교체 (28개 엔드포인트)
   - Step 11: `BATCH_SIZE` 5→3 (Neon 풀 5 고려)
   - Step 12: Puppeteer 제거 + `chart_path` 정리
   - Step 13: SQLite → Neon 데이터 마이그레이션 + 검증
   - Step 14: `dataFreshness.ts` 주석 업데이트 (로직-5)
   - **DB 호스팅**: Neon 무료 플랜 ($0). 매일 08:00 syncAllStocks가 sleep 방지
2. [ ] **`scripts/backfill-history.js` 작성** (PG 전환과 병행 — 독립 스크립트라 DB 레이어 무관). 97종목 × 3년 ≈ 72,750건, 종목당 1초 딜레이, 하루 20~30종목 분할, 체크포인트
3. [ ] **Puppeteer(`scrapers/toss.js`) 제거**: PG 전환과 함께. 이미 수집된 stock_history OHLCV로 Recharts 차트 렌더링 → `chart_path` 컬럼 불필요, Render에서 Chromium 설치 회피
4. [ ] KIS/KRX 평가 (시나리오 B 우선) — 장기로 이동

> **HMAC 서명은 Phase 5로 이동** (유료 구독 도입 시점). 사용자 0~수 명 단계에서는 device_id만으로 충분.

### Phase 3 - 빠른 웹앱 배포 (PostgreSQL 전환 직후)
PG 전환 완료 → Recharts lazy → Vercel + Render 배포 → 2~4주 운영 → Capacitor.
...
- [ ] **device_subscriptions 테이블** (PostgreSQL):
  ```sql
